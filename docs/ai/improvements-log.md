...
| 2026-06-23 | api-map.md (new) · map-api skill (new) · execution-policy · execute-test-cases · app-map | Dedicated API map + map-api skill; seed via API by default from the reusable catalog | API seeding was ad-hoc and re-discovered each run | Isadora |
| 2026-06-23 | generate-test-cases · coverage-report · AGENTS | Impact-based regression for related UCs: name the impact point and design a targeted regression case (graph-traversed, 1-2 hops), plus core re-run; coverage report shows regression gaps | Regression in related UCs was just happy-path re-runs and missed the new feature's actual impact | Isadora |
