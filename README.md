...
**Test pipeline**
- `generate-test-cases` — for one chosen use case: understand it + related UCs deeply, cover every SRS statement and dimension using formal test-design techniques (equivalence/boundary/decision-table/state/pairwise) with concrete values + precise oracles, role-based + dependency-aware, link to baseline cases, adversarial `/goal` loop; a pre-gen testability gap-check; saved as **step-per-row CSV** (step-level results)
- `execute-test-cases` — run via Playwright MCP; optional parallel runners (isolated by browser context, not account); create needed data via Admin (assume input values, API or UI); fixtures; verify from persisted state; `/goal` loop; run report (PASS / FAIL / BLOCKED / INCONCLUSIVE / FLAKY)
